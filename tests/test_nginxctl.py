...
NGINXCTL_OUTPUT = textwrap.dedent("""\
    \033[1mnginx vhost configuration:\033[0m
    *:8080 is a Virtualhost
    \tport 8080 namevhost \033[92m default_server_name \033[0m (/etc/nginx/nginx.conf:37)
    \t\talias \x1b[96m default_alias \x1b[0m
    [::]:8080 is a Virtualhost
    \tport 8080 namevhost \033[92m default_server_name \033[0m (/etc/nginx/nginx.conf:37)
    \t\talias \x1b[96m default_alias \x1b[0m
    """)
...
def test_nginxctl_output(monkeypatch, capsys):
    def mockreturn(_get_vhosts):
        vhosts_list = [{'l_num': 37, 'alias': ['default_alias'], 'servername': 'default_server_name', 'config_file': '/etc/nginx/nginx.conf', 'ip_port': ['8080', '[::]:8080']}]
        return vhosts_list
    monkeypatch.setattr(nginxctl.nginxCtl, '_get_vhosts', mockreturn)
    n.get_vhosts()
    out, err = capsys.readouterr()
    assert str(out) == NGINXCTL_OUTPUT
